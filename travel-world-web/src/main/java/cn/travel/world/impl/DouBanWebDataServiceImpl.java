...
@Service
public class DouBanWebDataServiceImpl implements WebDataService {
...
//			descResp.setAuthorPic(e.select("dl > dd > div.post-info > div.author >img[src]").attr("src"));
	...
	public Object getDataDetail(QueryDetailsReq req) {
		...
		String json= KK.http.sendGet(String.format(MOVIES_DETAILS_URL,req.getId(),PAGE_LIMIT,PAGE_LIMIT*(req.getPage()-1)),input);
...
public static void main(String[] args) {
	...
	req.setPage(1);
	req.setId("3878007");
	new DouBanWebDataServiceImpl().getDataDetail(req);
}
}
